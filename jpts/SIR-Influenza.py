...
def fitErrorSIR(trial, fluSeries, t_range):
    ...
    fSeries_norm = scaler.fit_transform(fSeries).flatten()
    SIR_Res_norm = scaler.transform(SIR_Res).flatten()

    return mean_squared_error(fSeries_norm, SIR_Res_norm)
...
if __name__ == '__main__':
    ...
    for year in ausPop.keys():
        try:
            ...
            if len(fluSeries[startdate:enddate]) != len(t_range):
                continue
            # Initial conditionss
            ...
            study.optimize( lambda trial: 
                            fitErrorSIR(trial, fluSeries, t_range),
                            n_trials = n_trials)
